...
export const getRoom = asyncHandler(async(req,res)=>{
    ...
    if(!room.isPublic){
        const isMember = room.participants.some(p => p.userId.toString() === req.user._id.toString())
        if(!isMember){
...
export const joinRoom = asyncHandler(async(req,res)=>{
    ...
    const alreadyIn = room.participants.some(p => p.userId.toString() === req.user._id)
    if(!alreadyIn){
...
export const updateLanguage = asyncHandler(async(req,res)=>{
    ...
    if(!roomId || !language)return res.status(401).json({message:`both roomId:${roomId} and language: ${language} needed`})
    const room = await Room.findOne({roomId})
    if(!room)
        return res.status(401).json({message:"Invalid roomId given to change the language"})
    const isMember = room.participants.some(p => p.userId.toString() === req.user._id.toString())
    if(!isMember || isMember.role == "viewer"){
        return res.status(401).json({message:`you not a member of this room ${room.name} to change the language`})
    }
    room.language = language
    await room.save()
    return res.status(200).json({ message:`langusge change succsessfully for room ${room.name} by ${isMember.userId}` })
})
...
export const deleteRoom = asyncHandler(async (req,res)=>{
    const roomId = req.params.roomId
    if(!roomId){
        return res.status(401).json({message:"roomId require  for delete a room"})
    }
    const room = await Room.findOne({roomId})
    if(!room){
        return res.status(403).json({message:"No room found for given room id for deletion"})
    }
    if(room.owner.toString() !== req.user._id.toString()){
        return res.status(401).json({message:"Only owner have rights to delete the room"})
    }
    const deletedRoom = await Room.deleteOne({roomId})
    return res.status(201).json({message:`Room deleted for roomID: ${roomId} and name :${room.name}`})
})